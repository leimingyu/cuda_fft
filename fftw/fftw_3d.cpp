...
int main(int argc, char **argv)
{
	...
	printf("[LOG] Start 3d-fft CPU.\n");
	/*
	 * Parameters
	 */
	fftwf_complex *input, *output; // fp32

	input  = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sig_len * sig_len * sig_len);
	output = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sig_len * sig_len * sig_len);
	 ...
	 * fftw plan 
	 */
	printf("[LOG] Create C2C plan for %i x %i x %i on CPU.\n", sig_len, sig_len, sig_len);

	fftwf_plan plan_fp32_3d;

	plan_fp32_3d = fftwf_plan_dft_3d(sig_len, 
			sig_len, 
			sig_len,
			input, 
			output, 
	 ...
	 * Free 
	 */
	fftwf_destroy_plan(plan_fp32_3d);
