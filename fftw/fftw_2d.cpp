...
int main(int argc, char **argv)
{
	...
	printf("[LOG] Start 2d-fft CPU.\n");
	/*
	 * Parameters
	 */
	fftwf_complex *input, *output; // fp32

	input  = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sig_len * sig_len);
	output = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * sig_len * sig_len);
	 ...
	 * fftw plan 
	 */
	printf("[LOG] Create C2C plan for %i x %i on CPU.\n", sig_len, sig_len);

	fftwf_plan plan_fp32_2d;

	plan_fp32_2d = fftwf_plan_dft_2d(sig_len, 
			sig_len, 
			input, 
			output, 
			FFTW_FORWARD, 
	 ...
	 * Free 
	 */
	fftwf_destroy_plan(plan_fp32_2d);
